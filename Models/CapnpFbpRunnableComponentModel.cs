...
public class CapnpFbpRunnableComponentModel : CapnpFbpComponentModel {
    ...
    public override async Task StartProcess(ConnectionManager conMan) {
        try {
            if (
                Editor.CurrentChannelStarterService == null || RunnableFactory == null || ProcessStarted
            ) {
                return;
            }
            ...
            if (!configInPortConnected && !string.IsNullOrWhiteSpace(ConfigString)) {
                ...
                if (_confIipOutPort.WriterSturdyRef == null) {
                    if (_confIipOutPort.RetrieveWriterFromChannelTask != null) {
                        ...
                        (_confIipOutPort.Writer, _confIipOutPort.WriterSturdyRef) =
                            await Shared.Shared.GetNewWriterFromChannel(_configInPort.Channel,
                                cancelToken);
                    }
                }
                ...
                if (_confIipOutPort.Writer == null) {
                    Debug.Assert(_confIipOutPort.Writer != null,
                    ...
                    _confIipOutPort.Writer =
                        await conMan.Connect<Channel<IP>.IWriter>(_confIipOutPort.WriterSturdyRef);
                }
                ...
                await _confIipOutPort.Writer.Write(
                    new Channel<IP>.Msg {
                        Value = new IP {
                            Content = new StructuredText { TheType = StructuredText.Type.json, Value = ConfigString }
                        }
                    },
                    cancelToken);
            ...
            await _portInfosWriter.Write(
                new Channel<PortInfos>.Msg { Value = new PortInfos { InPorts = inPortSRs, OutPorts = outPortSRs }, },
                cancelToken);
            ...
            Console.WriteLine($"{ProcessName}: CapnpFbpRunnableComponentModel::StartProcess: Caught exception: {e}");
        }
    }
    ...
    public override async Task StopProcess(ConnectionManager conMan) {
        try {
            ...
            Console.WriteLine($"{ProcessName}: CapnpFbpRunnableComponentModel::StopProcess: Caught exception: {e}");
        }
    }

    public override async Task CancelAndDisposeRemoteComponent() {
        Console.WriteLine($"{ProcessName}: CapnpFbpRunnableComponentModel::CancelAndDisposeRemoteComponent");
        ...
        //stop remote process
        Console.WriteLine(
            $"{ProcessName}: CapnpFbpRunnableComponentModel::CancelAndDisposeRemoteComponent stopping runnable/process");
        await _portInfosWriter.Write(new Channel<PortInfos>.Msg { which = Channel<PortInfos>.Msg.WHICH.Done });
        await Task.Delay(500); // let the process stop
        ProcessStarted = !(await Runnable.Stop());

        Console.WriteLine(
            $"{ProcessName}: CapnpFbpRunnableComponentModel::CancelAndDisposeRemoteComponent stopped runnable/process (ProcessStarted: {ProcessStarted})");
        Runnable?.Dispose();
        Runnable = null;
    }

    public override void Dispose() {
        Console.WriteLine($"{ProcessName}: CapnpFbpRunnableComponentModel::Dispose");
        base.Dispose();
    ...
    public override void FreeRemoteChannelsAttachedToPorts() {
        Console.WriteLine($"{ProcessName}: CapnpFbpRunnableComponentModel::FreeRemoteChannelsAttachedToPorts");

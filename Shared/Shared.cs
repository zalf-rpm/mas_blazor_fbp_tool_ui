...
public class Shared
{
    public static readonly ulong ChannelStarterInterfaceId = typeof(IStartChannelsService)
        .GetCustomAttribute<TypeIdAttribute>(false)?.Id ?? 0;

    public static readonly ulong RegistryInterfaceId = typeof(IRegistry)
        .GetCustomAttribute<TypeIdAttribute>(false)?.Id ?? 0;
    ...
    public static string MakeUniqueKey<T>(Dictionary<string, T> dict, string key)
    {
        ...
        while (dict.ContainsKey(key))
        {
            ...
            {
                if (int.TryParse(key2[^2..^1], out var i2)) key2 = key2[..^2] + (i2 + 1);
            }
        }
    ...
    public static Task CreateChannel(ConnectionManager conMan,
        IStartChannelsService css, PortModel outPort, CapnpFbpPortModel inPort)
    {
        ...
        var t = Task.Run(async () =>
        {
            if (inPort.Channel == null) // there is no channel for the IN port yet
            {
                ...
                inPort.Channel =
                    await conMan.Connect<IChannel<IP>>(si.Item1[0].ChannelSR);
